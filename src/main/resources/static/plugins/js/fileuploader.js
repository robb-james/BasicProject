 ...
 * 
 * Have ideas for improving this JS for the general community? Submit your
 * changes at: https://github.com/Valums-File-Uploader/file-uploader
 * 
 * VERSION 2.0 beta Original version 1.0 © 2010 Andrew Valums (
 * andrew(at)valums.com ) Licensed under GNU GPL 2 or later and GNU LGPL 2 or
...
qq.FileUploaderBasic = function(o) {
	this._options = {
		...
		// validation
		allowedExtensions : [],
		acceptFiles : null, // comma separated string of mime-types for browser
		// to display in browse dialog
		sizeLimit : 0,
		minSizeLimit : 0,
		abortOnFailure : true, // Fail all files if one doesn't meet the
		// criteria
		// events
		...
		// messages
		messages : {
			// typeError: "Unfortunately the file(s) you selected weren't the
			// type we were expecting. Only {extensions} files are allowed.",
			typeError : "文件类型错误",
			sizeError : "图片过大，上传失败，不可超过1M.", //"{file} 太大了, 最大文件不能超过 {sizeLimit}."
	...
	_validateFile : function(file) {
		...
		if (file.value) {
			...
			name = (file.fileName !== null && file.fileName !== undefined) ? file.fileName
				: file.name;
			size = (file.fileSize !== null && file.fileSize !== undefined) ? file.fileSize
				: file.size;
		}
...
qq.FileUploader = function(o) {
	...
	this._options.template = this._options.template.replace(/\{dragText\}/g,
		this._options.dragText);
	this._options.template = this._options.template.replace(
		/\{uploadButtonText\}/g, this._options.uploadButtonText);
	this._options.fileTemplate = this._options.fileTemplate.replace(
		/\{cancelButtonText\}/g, this._options.cancelButtonText);
	this._options.fileTemplate = this._options.fileTemplate.replace(
		/\{failUploadtext\}/g, this._options.failUploadText);
...
qq.UploadDropZone.prototype = {
	...
	_attachEvents : function() {
		...
		self._attach(self._element, 'dragleave',
			function(e) {
				if (!self._isValidFileDrag(e))
					return;
		...
		qq.UploadHandlerXhr.prototype,
		{
			...
			_upload : function(id, params) {
				...
				if (this._options.encoding == 'multipart') {
					...
					// NOTE: return mime type in xhr works on chrome
					// 16.0.9 firefox 11.0a2
					xhr.setRequestHeader("X-Mime-Type", file.type);
				}
				for (key in this._options.customHeaders) {
					xhr.setRequestHeader(key,
						this._options.customHeaders[key]);
				}
				;
				xhr.send(file);
			},
